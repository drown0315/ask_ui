...
- [x] Store bridge session state with `vmServiceUri` and `projectRoot`.
- [ ] Configure Flutter Inspector pub root directories during session creation.
- [ ] Add bridge endpoint for `GET /api/sessions/:sessionId/widget-tree`.
- [ ] Fetch Flutter Inspector summary tree with `isSummaryTree=true`.
- [ ] Normalize Flutter Diagnostics nodes into Ask UI `WidgetTreeNode`.
...
- [ ] Render real Widget Tree data in the existing panel.
...
- Added bridge tests for missing parameters and successful session creation.
