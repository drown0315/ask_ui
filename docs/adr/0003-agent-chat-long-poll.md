...
Agent replies are written back through Agent Session Commands, with the standard loop shaped as poll, process, reply, and poll again. A combined CLI command such as `agent poll --reply-to <message-id> --agent-reply "..."` can write the reply and immediately wait for the next message; command-level failures use a separate error path that writes a system message rather than pretending to be an agent reply.
